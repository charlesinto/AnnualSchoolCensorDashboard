...
// https://gist.githubusercontent.com/segebee/7dde9de8e70a207e6e19/raw/90c91f7318d67c9534e3a4d74e4bd755b144e01e/gistfile1.txt
 ...
 places: any[] = [
  ...
  getstates(): any[] {
    const states = []
    this.places.forEach(({state: { name , id}}) => {
      states.push({
        text: name,
        id: id
      })
  ...
  getLocals(id : any[] = []): any[] {
    let lgas = [];
    const response = [];
    if(id.length === 0) {
      for(let i = 0; i < this.places.length; i++) {
        lgas = [];
        this.places[i].state.locals.forEach(elem => {
          lgas.push({
            text: elem.name,
            id: `${this.places[i].state.id}-${elem.id}`,
          })
        })
        response.push({
          text: this.places[i].state.name,
          id: this.places[i].state.id,
          children:lgas
        })
      }
      return response;
    ...
    for(let i = 0; i < id.length; i++) {
      const index = this.places.findIndex(item => item.state.id === parseInt(id[i]));
      lgas = [];
      this.places[index].state.locals.forEach(elem => {
        lgas.push({
          text: elem.name,
          id: `${this.places[index].state.id}-${elem.id}`,
        })
      })
      response.push({
        text: this.places[index].state.name,
        id: index,
        children:lgas
      })
    }
